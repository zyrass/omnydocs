...
!!! quote "Analogie pédagogique"
    _Concevoir et administrer un réseau, c'est comme concevoir le système routier d'un pays. Les routeurs sont les échangeurs d'autoroute, les switchs sont les ronds-points locaux, et les pare-feu sont les postes de douane. Un bon réseau doit permettre une circulation fluide tout en interdisant l'accès aux zones sensibles._
...
_Explication : ARP est utilisé pour mapper les adresses IP aux adresses MAC sur un réseau local._
...
_Explication : AS est défini comme : groupe de réseaux IP placés sous une administration technique et politique commune._
...
_Explication : ASN est défini comme : identifiant numérique unique attribué à chaque système autonome par un registre Internet régional._
...
_Explication : BGP est défini comme : protocole de routage externe utilisé pour échanger des routes entre systèmes autonomes sur Internet._
...
_Explication : Broadcast est défini comme : mode de transmission envoyant des données à tous les hôtes d'un même segment réseau simultanément._
...
_Explication : CIDR est défini comme : méthode d'allocation d'adresses IP utilisant une notation de préfixe variable pour optimiser l'espace d'adressage._
...
_Explication : Collision domain est défini comme : zone réseau où les trames peuvent entrer en collision lors de transmissions simultanées sur le même support._
...
_Explication : DMZ est défini comme : zone réseau intermédiaire isolée, exposant des services publics sans compromettre le réseau interne._
...
_Explication : DNS est défini comme : système hiérarchique distribué assurant la résolution des noms de domaine en adresses IP._
...
_Explication : Ethernet est défini comme : standard de réseau local filaire le plus répandu, basé sur la trame Ethernet et la commutation de paquets._
...
_Explication : FQDN est défini comme : nom de domaine complet spécifiant la position exacte d'un hôte dans la hiérarchie DNS._
...
_Explication : Gateway est défini comme : dispositif réseau assurant la passerelle entre deux réseaux utilisant des protocoles différents ou des espaces d'adressage distincts._
...
_Explication : HSRP est défini comme : protocole propriétaire Cisco de redondance permettant à plusieurs routeurs de partager une adresse IP virtuelle commune._
...
_Explication : ICMP est défini comme : protocole de messages de contrôle et d'erreur intégré à la suite TCP/IP pour les diagnostics réseau._
...
_Explication : IGP est défini comme : protocole de routage utilisé à l'intérieur d'un système autonome pour distribuer les routes internes._
...
_Explication : LACP est défini comme : protocole standard de négociation pour l'agrégation de plusieurs liens physiques en un lien logique unique._
...
_Explication : Latency est défini comme : délai de transmission des données entre un émetteur et un récepteur sur le réseau._
...
_Explication : MAC Address est défini comme : identifiant matériel unique de 48 bits attribué à chaque interface réseau pour l'adressage au niveau liaison de données._
...
_Explication : MTU est défini comme : taille maximale en octets d'un paquet transmissible sur un lien réseau sans fragmentation._
...
_Explication : Multicast est défini comme : mode de transmission envoyant des données à un groupe spécifique d'abonnés, sans dupliquer inutilement le trafic._
...
_Explication : NAT est défini comme : technique de traduction des adresses IP permettant à des réseaux privés d'accéder à Internet via une adresse publique partagée._
...
_Explication : NetFlow est défini comme : protocole de collecte et d'analyse des métadonnées de trafic réseau pour le monitoring et la sécurité._
...
_Explication : OSPF est défini comme : protocole de routage à état de liens utilisant l'algorithme de Dijkstra pour calculer les chemins optimaux._
...
_Explication : Packet est défini comme : unité de données formatée transmise sur un réseau à commutation de paquets, comprenant des en-têtes et le payload._
...
_Explication : Port mirroring est défini comme : technique de copie du trafic d'un ou plusieurs ports de switch vers un port d'analyse dédié._
...
_Explication : QoS est défini comme : ensemble de mécanismes gérant la qualité et la priorité du trafic réseau pour garantir les performances des applications critiques._
...
_Explication : RIP est défini comme : protocole de routage à vecteur de distance utilisant le nombre de sauts comme métrique unique._
...
_Explication : Route est défini comme : entrée dans la table de routage définissant le chemin qu'un paquet doit emprunter pour atteindre un réseau de destination._
...
_Explication : Spanning Tree est défini comme : protocole réseau évitant les boucles de commutation dans les infrastructures de switches redondants._
...
_Explication : Subnet est défini comme : subdivision logique d'un réseau IP en segments plus petits pour améliorer organisation, sécurité et performances._
...
_Explication : Switch est défini comme : équipement de commutation opérant au niveau 2 du modèle OSI pour interconnecter les hôtes d'un réseau local._
...
_Explication : Trunking est défini comme : technique permettant de transporter le trafic de plusieurs VLANs sur un seul lien physique entre équipements réseau._
...
_Explication : TTL est défini comme : valeur numérique décrémentée à chaque saut réseau empêchant les paquets de circuler indéfiniment._
...
_Explication : Unicast est défini comme : mode de transmission point à point entre un émetteur unique et un récepteur unique._
...
_Explication : VLAN est défini comme : réseau local virtuel segmentant logiquement un réseau physique en groupes d'hôtes isolés, indépendamment de leur position physique._
...
_Explication : VRRP est défini comme : protocole standard ouvert de redondance de passerelle, permettant à plusieurs routeurs de partager une adresse IP virtuelle._
...
_Explication : VTP est défini comme : protocole propriétaire Cisco gérant la synchronisation centralisée des informations VLAN entre switches d'un même domaine._
...
_Explication : WAN est défini comme : réseau étendu couvrant une large zone géographique pour interconnecter des sites distants ou des organisations._
