...
**Créer système auth complet avec toutes protections**
...
// Classes complètes (voir sections précédentes)
require_once 'classes/Auth.php';
require_once 'classes/RememberMe.php';
...
Voir fichiers complets dans solution exercice (trop volumineux pour inclusion ici).

</details>

### Exercice 2 : API Authentification avec JWT

**Créer API RESTful avec authentification JWT**
...
/**
 * API REST avec JWT (JSON Web Tokens)
 * 
 * Endpoints :
 * POST /api/register - Inscription
 * POST /api/login - Connexion (retourne JWT)
 * GET /api/profile - Profil (requiert JWT)
...
class JWT {
     ...
     * Générer token JWT
     */
    public function generer(array $payload, int $expiration = 3600): string {
        $header = [
            'alg' => 'HS256',
            'typ' => 'JWT'
        ];
