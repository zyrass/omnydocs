...
**Créer classe gérant fichiers avec gestion erreurs complète**
...
class FileManager {
    public function read(string $path): string {
        ...
        if (!is_readable($path)) {
            throw new FileNotReadableException("Fichier non lisible : $path");
        }
        
        $content = file_get_contents($path);
        
        if ($content === false) {
            throw new FileException("Erreur lors de la lecture de : $path");
        }
    ...
    public function write(string $path, string $content): void {
        ...
        if (!is_dir($directory)) {
            if (!mkdir($directory, 0755, true)) {
                throw new DirectoryNotWritableException("Impossible de créer le répertoire : $directory");
            }
        }
        
        if (file_exists($path) && !is_writable($path)) {
            throw new FileNotWritableException("Fichier non modifiable : $path");
        }
        
        if (file_put_contents($path, $content) === false) {
            throw new FileException("Erreur lors de l'écriture dans : $path");
        }
    }
    ...
    public function delete(string $path): void {
        ...
        if (!is_writable(dirname($path))) {
            throw new DirectoryNotWritableException("Répertoire non modifiable : " . dirname($path));
        }
        
        if (!unlink($path)) {
            throw new FileException("Erreur lors de la suppression de : $path");
        }
    }
    
    public function copy(string $source, string $destination): void {
        if (!file_exists($source)) {
            throw new FileNotFoundException("Fichier source introuvable : $source");
        }
        ...
        if (!is_dir($destinationDir)) {
            if (!mkdir($destinationDir, 0755, true)) {
                throw new DirectoryNotWritableException("Impossible de créer : $destinationDir");
            }
        }
        
        if (!copy($source, $destination)) {
            throw new FileException("Erreur lors de la copie de $source vers $destination");
        }
    }
}
...
try {
    // Lecture
    $content = $fileManager->read('data.txt');
    echo "Contenu : $content\n";
    
    // Écriture
    $fileManager->write('output/result.txt', 'Hello World');
    echo "Écriture réussie\n";
    
    // Copie
    $fileManager->copy('data.txt', 'backup/data.txt');
    echo "Copie réussie\n";
    ...
    echo "Fichier introuvable : " . $e->getMessage() . "\n";
    
} catch (FileNotReadableException $e) {
    echo "Fichier non lisible : " . $e->getMessage() . "\n";
    
} catch (DirectoryNotWritableException $e) {
    echo "Répertoire non modifiable : " . $e->getMessage() . "\n";
...
**Créer client API avec gestion erreurs et retry automatique**
...
class ApiClient {
    ...
    private function executeRequest(string $method, string $endpoint, ?array $data): array {
        ...
        if ($response === false) {
            // Analyser erreur HTTP
            $error = error_get_last();
            
            if (str_contains($error['message'], 'timeout')) {
                throw new ApiTimeoutException("Timeout lors de la requête vers $url");
            }
            
            throw new ApiException("Erreur HTTP vers $url");
        }
        ...
        try {
            ...
            throw new ApiException("Réponse JSON invalide", 0, $e);
        }
    }
}

// Tests
$client = new ApiClient('https://api.example.com', 'api_key_123');
