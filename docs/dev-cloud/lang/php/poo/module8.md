...
**Créer classe complète pour article de blog**
...
class BlogPost {
    ...
    public function setTitle(string $title): void {
        if (strlen($title) < 5 || strlen($title) > 200) {
            throw new InvalidArgumentException("Titre doit faire entre 5 et 200 caractères");
        }
        $this->title = $title;
    }
    ...
    public function setContent(string $content): void {
        if (strlen($content) < 50) {
            throw new InvalidArgumentException("Contenu doit faire minimum 50 caractères");
        }
...
$post1 = new BlogPost(
    ...
    'La Programmation Orientée Objet est un paradigme qui organise le code autour d\'objets. ' .
    'Les objets combinent données et comportements dans des structures cohérentes. ' .
    'Cette approche améliore la réutilisabilité et la maintenabilité du code.',
    'Alice'
);

echo "ID: " . $post1->getId() . "\n";
echo "Titre: " . $post1->getTitle() . "\n";
echo "Auteur: " . $post1->getAuthor() . "\n";
echo "Statut: " . $post1->getStatus() . "\n";
echo "Temps lecture: " . $post1->getReadingTime() . " min\n";
echo "Extrait: " . $post1->getExcerpt(100) . "\n";

$post1->publish();
echo "Statut après publication: " . $post1->getStatus() . "\n";
echo "Date publication: " . $post1->getPublishedAt()->format('Y-m-d H:i:s') . "\n";

$post1->incrementViews();
$post1->incrementViews();
$post1->incrementViews();
echo "Vues: " . $post1->getViews() . "\n";

echo "Total articles: " . BlogPost::getTotalPosts() . "\n";
```
...
**Créer classes pour comptes bancaires avec transactions**
...
class BankAccount {
    ...
    public function transfer(BankAccount $destination, float $amount): bool {
        // Débiter ce compte
...
// Usage
$account1 = new BankAccount('FR123', 'Alice', 1000.00);
$account2 = new BankAccount('FR456', 'Bob', 500.00);
