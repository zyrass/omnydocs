...
**Créer hiérarchie pour gérer différents types de médias**
...
class VideoMedia extends Media {
    ...
    public function display(): string {
        return "🎥 Vidéo : {$this->title} ({$this->resolution}) - {$this->formatDuration()}";
    }
...
class AudioMedia extends Media {
    ...
    public function display(): string {
        return "🎵 Audio : {$this->title} ({$this->format}) - {$this->formatDuration()}";
    }
...
class MediaLibrary {
    ...
    public function getMostViewed(): ?Media {
        if (empty($this->medias)) {
            return null;
        }
        
        return array_reduce($this->medias, function($max, $media) {
            return ($max === null || $media->getViews() > $max->getViews()) ? $media : $max;
        });
    }
}
...
$video = new VideoMedia('PHP Tutorial', 'Alice', 1800, '4K');
$video->incrementViews();
$video->incrementViews();
$video->incrementViews();

$audio = new AudioMedia('Podcast Tech', 'Bob', 2400, 'mp3');
$audio->incrementViews();

$article = new TextMedia(
    'Introduction POO',
    'Charlie',
...
echo "\nDurée totale : " . $library->getTotalDuration() . " secondes\n";

$mostViewed = $library->getMostViewed();
echo "Plus vu : " . $mostViewed->getTitle() . " ({$mostViewed->getViews()} vues)\n";
```
...
**Créer système de personnages de jeu avec héritage**
...
abstract class Character {
    ...
    public function gainExperience(int $exp): void {
        // Ajouter XP, level up si nécessaire
    }
}
...
class Mage extends Character {
    ...
    public function castSpell(Character $target, string $spell): int {
        // Sort spécial
    }
...
class Archer extends Character {
    ...
    public function criticalShot(Character $target): int {
        // Tir critique
    }
...
// Combat
$warrior = new Warrior('Conan', 150);
$mage = new Mage('Gandalf', 80);

while ($warrior->isAlive() && $mage->isAlive()) {
    $damage1 = $warrior->attack($mage);
    echo "Conan inflige $damage1 dégâts\n";
    ...
    $damage2 = $mage->attack($warrior);
    echo "Gandalf inflige $damage2 dégâts\n";
}
```
