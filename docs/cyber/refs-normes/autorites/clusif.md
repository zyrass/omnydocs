...
---
graph TB
    A[Écosystème français<br/>de la cybersécurité]
    
    B[Autorités publiques<br/>ANSSI, CNIL]
    C[Associations professionnelles<br/>CLUSIF, CESIN, AFCDP]
    D[Organismes de formation<br/>Universités, écoles, centres]
    E[Entreprises privées<br/>Éditeurs, SSII, cabinets]
    ...
    C --> C1[CLUSIF<br/>Généraliste SSI]
    C --> C2[CESIN<br/>RSSI grandes entreprises]
    C --> C3[AFCDP<br/>Délégués protection données]
    
    B -.->|Référentiels officiels| C
    C -.->|Retours terrain| B
    
    C -.->|Contenus pédagogiques| D
    D -.->|Recherche appliquée| C
    
    C -.->|Bonnes pratiques| E
    E -.->|Expertise technique| C
    
    style A fill:#e3f3e3
    style C fill:#f3e3e3
    style C1 fill:#e3e3f3
```
...
graph TD
    ...
    C[Bureau<br/>Président, VP, Trésorier]
    D[Commission Scientifique<br/>Experts thématiques]
    
    E[Groupes de Travail<br/>Thématiques spécialisés]
    F[Délégations Régionales<br/>Représentation territoriale]
...
---
graph LR
    A[Identification<br/>d'un besoin] --> B[Constitution<br/>du GT]
    B --> C[Définition du<br/>périmètre et objectifs]
    C --> D[Réunions de travail<br/>mensuelles ou bimestrielles]
    D --> E[Production de<br/>livrables intermédiaires]
    E --> F{Validation<br/>Commission<br/>Scientifique}
    F -->|Corrections| D
    F -->|Approuvé| G[Publication<br/>officielle]
    G --> H[Diffusion auprès<br/>des membres]
    H --> I[Communication<br/>publique]
...
---
graph LR
    A[ANSSI] -->|Référentiels<br/>officiels| B[CLUSIF]
    B -->|Retours<br/>terrain| A
    
    A -->|Experts dans<br/>GT CLUSIF| B
    B -->|Relais des<br/>recommandations| A
    
    A -->|Financement<br/>études| B
    B -->|Données<br/>statistiques| A
    
    C[Communauté<br/>professionnelle]
    
    A -.->|Sensibilisation<br/>via CLUSIF| C
    B -.->|Formation<br/>continue| C
...
graph TD
    ...
    C --> C1[Statistiques d'incidents<br/>secteur similaire]
    C --> C2[Coûts moyens<br/>des incidents]
    C --> C3[Temps de détection<br/>et réponse]
    C --> C4[Tendances<br/>d'évolution]
    
    C1 --> D[Quantification<br/>du risque]
    C2 --> D
    C3 --> E[Identification<br/>des gaps]
    C4 --> E
    
    D --> F[Présentation<br/>COMEX/CODIR]
    E --> F
    
    F --> G{Décision ?}
    G -->|Approuvé| H[Déploiement projet]
    G -->|Refusé| I[Notation du<br/>risque accepté]
