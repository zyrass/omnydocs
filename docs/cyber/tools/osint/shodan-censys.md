...
!!! quote "Ce qu'il faut retenir"
    Shodan et Censys sont les "yeux dans le ciel" de l'auditeur. Ils vous permettent de préparer votre attaque en sachant exactement quelles versions de logiciels tournent sur quels serveurs, et ce, sans lancer un seul scan de port. Ils sont le pont parfait entre la collecte d'information (OSINT) et la phase d'énumération active.

> Maintenant que vous avez identifié les adresses IP et les services exposés, utilisez des outils d'énumération actifs mais précis pour valider ces trouvailles, comme **[Subfinder →](./subfinder.md)**.
